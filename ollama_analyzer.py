...
def parse_astrasim_log(log_path: Path) -> dict:
    """
    Final, correct Astra-Sim log parser for the format provided:
    
      [workload] [info] sys[n] finished, <wall> cycles, exposed communication <comm> cycles.
      [statistics] [info] sys[n], Wall time: <wall>
      [statistics] [info] sys[n], Comm time: <comm>

    This parser guarantees:
      • wall_time always extracted
      • comm_time always extracted
      • compute_time calculated robustly
      • no KeyErrors ever
    """
    ...
    rank_ids = re.findall(r"sys\[(\d+)\]", text)
    for r in rank_ids:
        ranks_data[int(r)] = {"wall_time": None, "comm_time": None}

    wall_matches = re.findall(
        r"sys\[(\d+)\].*?Wall time:\s*([0-9]+)", text, flags=re.IGNORECASE
    )
    for r, w in wall_matches:
        ranks_data[int(r)]["wall_time"] = float(w)

    comm_matches = re.findall(
        r"sys\[(\d+)\].*?Comm time:\s*([0-9]+)", text, flags=re.IGNORECASE
    )
    for r, c in comm_matches:
        ranks_data[int(r)]["comm_time"] = float(c)
    fallback_matches = re.findall(
        r"sys\[(\d+)\]\s*finished,\s*([0-9]+)\s*cycles,.*?exposed communication\s*([0-9]+)\s*cycles",
        text,
        flags=re.IGNORECASE,
    )
    for r, w, c in fallback_matches:
        r = int(r)
        if ranks_data[r]["wall_time"] is None:
            ranks_data[r]["wall_time"] = float(w)
        if ranks_data[r]["comm_time"] is None:
            ranks_data[r]["comm_time"] = float(c)

    for r in ranks_data:
        if ranks_data[r]["wall_time"] is None:
            print(f"[WARNING] No wall_time found for rank {r}, setting to 0.")
            ranks_data[r]["wall_time"] = 0.0

        if ranks_data[r]["comm_time"] is None:
            print(f"[WARNING] No comm_time found for rank {r}, setting to 0.")
            ranks_data[r]["comm_time"] = 0.0

        ranks_data[r]["compute_time"] = max(
            0.0,
            ranks_data[r]["wall_time"] - ranks_data[r]["comm_time"]
        )
...
def simulate_what_if(metrics: dict, whatif_str: str) -> dict:
    ...
    if "topology" in params:
        ...
        if topo == "tree":
            new["comm_fraction"] *= 0.82     
        elif topo == "fc":
            new["comm_fraction"] *= 0.75    
            new["rank_imbalance_pct"] *= 0.9
        elif topo == "ring":
            new["comm_fraction"] *= 1.05    
...
def generate_recommendation(metrics, rules, dominant, similar_cases=None):
    """
    Generates an LLM-based expert analysis using the detected bottleneck
    and the R1–R10 rule system.
    """

    model = "llama3"
    rule_text = "\n".join([f"- [{r['rule']}] {r['type']}" for r in rules]) if rules else "None"

    dominant_type = dominant["type"] if dominant else "None"
    dominant_rule = dominant["rule"] if dominant else "None"
    dominant_cause = dominant["interpretation"] if dominant else "None"
    dominant_mitigation = dominant["mitigation"] if dominant else "None"

    similar_text = ""
    if similar_cases:
        similar_text = json.dumps(similar_cases, indent=2)

    prompt = f"""
You are an expert in distributed deep-learning performance, GPU interconnects,
system bottleneck analysis, and communication-optimized training.

Provide a detailed analysis for the following system bottleneck.
...
=============================
Type: {dominant_type}
Rule Triggered: {dominant_rule}
Cause (from analyzer): {dominant_cause}
Recommended Mitigation (from analyzer): {dominant_mitigation}

=============================
REQUEST
=============================
1. Explain WHY this bottleneck occurs.
2. Describe the ROOT CAUSE clearly.
3. Tie your reasoning to the system context (compute/comm overlap, topology, BW, imbalance, etc).
4. Propose the BEST mitigation strategy.
5. Give 2–3 alternative improvements that could further benefit the system.
6. If similar past cases exist, compare them.

Respond in a clear, well-structured format with bullet points and section headers.
    """
...
def generate_dashboard_pdf(
    ...
    def add_graph_if_exists(name):
        if name in graph_paths:
            ...
            col1.append(Image(img_path, width=w_scaled, height=h_scaled))
    ...
    col2.append(Spacer(1, 10))
    col1_frame = KeepInFrame(
        maxWidth=300,
        maxHeight=500,  
        content=col1,
        mode='shrink'   
    )
    col2_frame = KeepInFrame(
        maxWidth=450,
        maxHeight=500,
        content=col2,
        mode='shrink'
...
def main():
    ...
    print("\n===============================")
    print(" LLM EXPERT ANALYSIS (FULL) ")
    print("===============================")
